...
- **Market news is filtered by asset, but not truly per-user.** `getMarketNews()` queries NewsData.io live on every dashboard load, using the user's onboarding `assets` as keywords (falling back to the unfiltered feed if that query returns nothing). This means: (1) two users with the same asset selections see identical news, since the query only depends on `assets`, not on user identity; (2) the same user can see different articles across reloads, since there's no caching or daily-pinning like the meme pool has — it's a fresh live query each time. `investorType`/`contentTypes` are still unused for news — a further fix would map content types to NewsData's `category` param or re-rank results client-side.
